...
- **about.txt reveal pacing changed.** The seed is now `"Capital P is a "` (trailing space) so the first keystroke reveals a real letter instead of a space. Reveals one character at a time through "...company offering" (enough to sell the gimmick), then switches to revealing a whole word per keystroke — mashing out the full ~340-character passage one character at a time got tedious. Backspace mirrors the same granularity; a word-jump never crosses a paragraph break in one keystroke.
- **Background video dims to black while reel.html is open**, fading back once it's closed — the looping video flickering behind the Vimeo embed was distracting.
- **Fixed a short-viewport clipping bug** (landscape phones especially): `.home-pane` and `.reel-pane` used `justify-content: center`, which clips overflowing content off both ends with the top portion permanently unreachable (scroll position can't go negative). Switched to `justify-content: safe center`, which falls back to top-alignment instead of centering once content would overflow.
- **A hint fades in on the boot screen** if `CAPITAL_P.EXE` sits unclicked for 5 seconds — `"(you're supposed to click it)"` on mouse, `"(tap it)"` on touch (keyed off `pointer: coarse`, not viewport width).
- **A sixth home icon, AI POLICY, was added** — not in the original 5-item handoff. Unlike every other icon on the page (CSS boxes, no images — see `src/glyphs.js`), this one is a client-supplied 512×512 PNG (`assets/ai-policy-icon-512.png`) used as-is: it already bakes in its own background at a color close enough to `--ink` not to seam against the CSS-drawn tiles beside it, so it's dropped straight into the icon grid rather than composited into the shared tile wrapper. Opens a **list of dated policy versions** (same pattern as WORK → project detail, or TEAM → bio: index window, then a detail window per item), not a single static page — currently one entry, "August 18, 2026", but the client wants old versions preserved unedited if the policy is ever updated, so `AI_POLICY_VERSIONS` in `src/data/content.js` is an array from the start; a future version is just a new entry at the front of that array, nothing else needs restructuring. The detail window reuses about.txt's framed-panel look (caption bar + white prose well) but with no reveal gimmick — it's just fully written. Opening an actual policy is 3 windows deep: home → AI POLICY list → the dated version. Registry status counts (`5 item(s)` → `6 item(s)`) updated to match.
- **Home icon grid is a fixed 3x2, not the original single-row `auto-fit`.** With a 6th icon the row got cramped and "AI POLICY" (the one two-word label) felt like the odd one out in a line of single words. `.home-pane` already had `justify-content: safe center` (see the short-viewport fix above), so the grid is free to take the vertical space a 2-row layout naturally wants — short viewports scroll to see the second row instead of anything being squeezed or clipped.
